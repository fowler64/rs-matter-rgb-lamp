...
use rs_matter_embassy::matter::dm::clusters::on_off::{
    self, ClusterAsyncHandler as _, NoLevelControl, OnOffHandler,
};
...
async fn main(_s: Spawner) {
    ...
    let light_output = Output::new(
        peripherals.GPIO5.degrade(),
        Level::High,
        OutputConfig::default(),
    );

    let light_handler = LightHandler::new(light_output);
    ...
    let handler = EmptyHandler
        .chain(
            EpClMatcher::new(
                ...
                Some(OnOffHandler::<LightHandler, NoLevelControl>::CLUSTER.id),
            ),
    ...
    // == Step 7: ==
    // Run async tasks
    match select(&mut matter, &mut pin!(reset_button_task())).await {
        Either::First(r) => {
    ...
    endpoints: &[
        ...
        Endpoint {
            ...
            clusters: clusters!(
                ...
                OnOffHandler::<LightHandler, NoLevelControl>::CLUSTER,
            ),
        },
    ],
};
